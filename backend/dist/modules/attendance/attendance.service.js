...
export class AttendanceService {
    ...
    static async recordAttendance(scannedBy, data, teacherId) {
        ...
        if (!sessionGroup)
            throw NotFoundException({ message: 'المجموعة المرتبطة بالحصة غير موجودة' });
    ...
    static async recordManualAttendance(scannedBy, studentId, teacherId) {
    ...
    static async batchRecordAttendance(scannedBy, data, teacherId) {
        const session = await SessionModel.findOne({ _id: data.sessionId, teacherId }).lean();
        if (!session)
            throw NotFoundException({ message: 'الحصة غير موجودة' });
        if (session.status === SessionStatus.COMPLETED) {
    ...
    static async getSessionAttendance(sessionId, teacherId, search) {
        ...
        filtered.sort((a, b) => {
            const nameA = a.studentId?.studentName ?? '';
            const nameB = b.studentId?.studentName ?? '';
            return nameA.localeCompare(nameB, 'ar');
        });
        return filtered;
    }
    // ─── Complete session + generate Snapshot ───────────────────────
    static async completeSession(sessionId, teacherId, completedBy) {
        const session = await SessionModel.findOne({ _id: sessionId, teacherId }).lean();
        if (!session)
            throw NotFoundException({ message: 'الحصة غير موجودة' });
        if (session.status === SessionStatus.COMPLETED) {
        ...
        const allStudents = await StudentModel.find({ groupId: session.groupId, teacherId, isActive: true }, { _id: 1, studentName: 1, excusedSessionsCount: 1, excusedUntil: 1, parentPhone: 1 }).sort({ studentName: 1 }).lean();
        ...
        const excusedAttendanceDocsToInsert = [];
        const absentAttendanceDocsToInsert = [];
        const studentIdsToDecrementExcuse = [];
        ...
        if (guestRecords.length > 0) {
            ...
            const guestStudentDocs = await StudentModel.find({ _id: { $in: guestIds } }, { studentName: 1 }).lean();
        ...
        const { updatedSession, snapshot } = await withTransaction(async (dbSession) => {
            ...
            if (studentIdsToDecrementExcuse.length > 0) {
                await StudentModel.updateMany({ _id: { $in: studentIdsToDecrementExcuse } }, { $inc: { excusedSessionsCount: -1 } }, { session: dbSession });
            }
            // Update session status + create snapshot
            const [updatedSession, snapshot] = await Promise.all([
                SessionModel.findByIdAndUpdate(sessionId, { status: SessionStatus.COMPLETED }, { new: true, session: dbSession }).lean(),
                AttendanceSnapshotModel.findOneAndUpdate({ sessionId }, {
                    sessionId,
                ...
                }, { upsert: true, new: true, session: dbSession }).lean(),
            ]);
            ...
            if (allPresentIds.length > 0) {
                await StudentModel.updateMany({ _id: { $in: allPresentIds }, remainingSessions: { $gt: 0 } }, { $inc: { remainingSessions: -1 } }, { session: dbSession });
            }
        ...
        if (absentStudents.length > 0) {
            ...
            const phoneMap = new Map(allStudents
                .filter(s => s.parentPhone)
                .map(s => [s._id.toString(), s.parentPhone]));
            // Fetch group name (lean — single field)
            const groupDoc = await GroupModel.findById(session.groupId, { name: 1 }).lean().catch(() => null);
            const groupName = groupDoc?.name ?? 'المجموعة';
            ...
            const rawTeacherName = teacherDoc?.name ?? '';
            const subject = teacherDoc?.subject;
            ...
            for (const absent of absentStudents) {
                ...
                if (!parentPhone)
                    continue; // skip if no phone on record
                enqueueWhatsApp({
                    kind: 'session_absent',
                    teacherId,
                    parentPhone,
    ...
    static async getSnapshot(sessionId, teacherId) {
        const session = await SessionModel.findOne({ _id: sessionId, teacherId }).lean();
        if (!session)
            throw NotFoundException({ message: 'الحصة غير موجودة' });
        const snapshot = await AttendanceSnapshotModel.findOne({ sessionId }).lean();
        if (!snapshot)
            throw NotFoundException({ message: 'لم يتم إنهاء هذه الحصة بعد' });
        return snapshot;
    }
    // ─── Update a single attendance record (manual edit by assistant) ──
    static async updateAttendance(attendanceId, updatedBy, status, teacherId, notes) {
        const record = await AttendanceModel.findById(attendanceId).lean();
        if (!record)
            throw NotFoundException({ message: 'سجل الحضور غير موجود' });
        ...
        if (record.sessionId) {
            ...
            if (!session)
                throw NotFoundException({ message: 'الحصة غير موجودة أو لا صلاحية لك عليها' });
            if (session.status === SessionStatus.COMPLETED) {
            ...
            if (!student)
                throw NotFoundException({ message: 'الطالب غير موجود أو لا صلاحية لك عليه' });
        }
        if (!Object.values(AttendanceStatus).includes(status)) {
            throw BadRequestException({ message: 'حالة الحضور غير صحيحة' });
        }
        return await AttendanceModel.findByIdAndUpdate(attendanceId, {
            status,
    ...
    static async getGroupHistory(groupId, teacherId, queryFilters = {}) {
    ...
    static async generateWhatsAppLinks(sessionId, teacherId) {
        ...
        if (!session)
            throw NotFoundException({ message: 'الحصة غير موجودة' });
        const groupName = session.groupId?.name || 'مجموعة غير معروفة';
        const teacherName = teacher?.name || '';
        // 1. Get all students in the group — sorted alphabetically
        const allStudents = await StudentModel.find({ groupId: session.groupId, teacherId, isActive: true }, { studentName: 1, parentPhone: 1 }).sort({ studentName: 1 }).lean();
        ...
        return allStudents.map(student => {
            ...
            if (isPresent) {
                ...
                message = PRESENT_TEMPLATES[Math.floor(Math.random() * PRESENT_TEMPLATES.length)];
            }
            else {
                ...
                message = ABSENT_TEMPLATES[Math.floor(Math.random() * ABSENT_TEMPLATES.length)];
            }
