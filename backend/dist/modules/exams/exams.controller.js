...
examsRouter.post('/', authorizeRoles(UserRole.teacher, UserRole.assistant), validate(createExamSchema), async (req, res, next) => {
    try {
...
examsRouter.get('/', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
...
examsRouter.get('/:id', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const exam = await ExamsService.getExamById(req.params['id'], teacherId);
...
examsRouter.put('/:id', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const exam = await ExamsService.updateExam(req.params['id'], teacherId, req.body);
...
examsRouter.patch('/:id/publish', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const exam = await ExamsService.publishExam(req.params['id'], teacherId);
...
examsRouter.delete('/:id', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        await ExamsService.deleteExam(req.params['id'], teacherId);
...
examsRouter.post('/:id/results', authorizeRoles(UserRole.teacher, UserRole.assistant), validate(recordResultSchema), async (req, res, next) => {
    try {
        const user = req.user;
        if (!user)
            return next(BadRequestException({ message: 'المستخدم غير موجود' }));
...
examsRouter.post('/:id/results/batch', authorizeRoles(UserRole.teacher, UserRole.assistant), validate(batchResultsSchema), async (req, res, next) => {
    try {
        const user = req.user;
        if (!user)
            return next(BadRequestException({ message: 'المستخدم غير موجود' }));
...
examsRouter.get('/:id/results', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const data = await ExamsService.getExamResults(req.params['id'], teacherId);
...
examsRouter.post('/ai/generate', authorizeRoles(UserRole.teacher, UserRole.assistant), upload.single('pdf'), async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        if (!req.file) {
            return next(new Error('الرجاء رفع ملف PDF'));
        }
        const { title, date, passingMarks, gradeLevel, groupIds, questionCount, difficulty, questionTypes, marksPerQuestion, } = req.body;
        const types = Array.isArray(questionTypes)
            ? questionTypes
            : [questionTypes ?? QuestionType.MCQ];
        const result = await AIExamService.generateFromPDF(teacherId, req.file.buffer, {
            title,
            date,
...
aiProxyRouter.post('/', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
