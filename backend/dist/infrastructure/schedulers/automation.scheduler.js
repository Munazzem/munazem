...
import { generateWeeklyReports, generatePaymentReminders } from '../../modules/automation/automation.service.js';
...
export async function startAutomationScheduler() {
    ...
    for (const schedule of SCHEDULES) {
        await automationQueue.upsertJobScheduler(schedule.name, { pattern: schedule.pattern }, { name: schedule.name });
    ...
    // Start worker
    const worker = new Worker('automation', processAutomationJob, { connection, concurrency: 1 });
