...
export class AdminService {
    ...
    static async getOverviewStats() {
        ...
        const [totalTeachers, activeTeachers, totalStudents, activeSubscriptions, expiredSubscriptions, newTeachersThisMonth, recentErrors,] = await Promise.all([
    ...
    static async getAllTenants(query) {
        ...
        const filter = { role: UserRole.teacher };
        if (query.search) {
            filter.$or = [
        ...
        for (const sub of subscriptions) {
            ...
            if (!subMap.has(key))
                subMap.set(key, sub); // keep latest
        }
    ...
    static async getTenantDetail(teacherId) {
        const now = new Date();
        const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    static async updateTenant(id, updateData) {
        const teacher = await UserModel.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true }).select('-password').lean();
        if (!teacher)
            throw NotFoundException({ message: 'Teacher not found' });
        return teacher;
    }
    ...
    static async setTenantStatus(teacherId, isActive) {
        return UserModel.findOneAndUpdate({ _id: teacherId, role: UserRole.teacher }, { isActive }, { new: true }).select('-password').lean();
    }
    ...
    static async getActivityFeed(query) {
        ...
        const teachers = await UserModel.find({ _id: { $in: tenantIds } }, { name: 1, centerName: 1 }).lean();
        const teacherMap = new Map(teachers.map(t => [t._id.toString(), t]));
        const enriched = logs.map(log => ({
            ...log,
            teacherName: teacherMap.get(log.tenantId.toString())?.name ?? null,
        }));
        return { data: enriched, pagination: { total, page, limit, totalPages: Math.ceil(total / limit) } };
    }
    ...
    static async updatePlanPrices(newPrices) {
        const settings = await PlatformSettingsModel.findOneAndUpdate({ key: 'PLAN_PRICES' }, { value: newPrices }, { new: true, upsert: true }).lean();
        return settings.value;
    }
    // ── Promo Codes ────────────────────────────────────────────────────────
    ...
    static async createPromoCode(data) {
        ...
        if (existing)
            throw BadRequestException({ message: 'كود الخصم موجود بالفعل' });
        return PromoCodeModel.create({
            ...data,
            code: data.code.toUpperCase()
        });
    }
    static async togglePromoCode(id) {
        const promo = await PromoCodeModel.findById(id);
        if (!promo)
            throw NotFoundException({ message: 'كود الخصم غير موجود' });
    ...
    static async validatePromoCode(code) {
        ...
        if (!promo)
            throw BadRequestException({ message: 'كود الخصم غير صحيح' });
        if (!promo.isActive)
            throw BadRequestException({ message: 'كود الخصم غير مفعل' });
        if (promo.expiresAt && promo.expiresAt < new Date())
            throw BadRequestException({ message: 'كود الخصم منتهي الصلاحية' });
        if (promo.maxUses && promo.usedCount >= promo.maxUses)
            throw BadRequestException({ message: 'تم تجاوز الحد الأقصى لاستخدام كود الخصم' });
        return promo;
    }
    // ── Announcements ──────────────────────────────────────────────────────
    ...
    static async toggleAnnouncement(id) {
        const ann = await AnnouncementModel.findById(id);
        if (!ann)
            throw NotFoundException({ message: 'الإشعار غير موجود' });
