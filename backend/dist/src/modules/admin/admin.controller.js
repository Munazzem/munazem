...
adminRouter.post('/test-automation', async (req, res, next) => {
    try {
        const { type, teacherId } = req.body;
        if (type === 'weekly_report')
            await generateWeeklyReports(teacherId, true);
        else if (type === 'payment_reminder')
            await generatePaymentReminders(teacherId);
        else
            return next(new Error('Invalid type. Use "weekly_report" or "payment_reminder"'));
...
adminRouter.get('/announcements/active', async (req, res, next) => {
    try {
...
// ── GET /admin/growth ────────────────────────────────────────────────
adminRouter.get('/growth', async (_req, res, next) => {
    try {
...
adminRouter.patch('/tenants/:id', async (req, res, next) => {
    try {
        const data = await AdminService.updateTenant(req.params['id'], req.body);
...
adminRouter.post('/tenants/:id/suspend', async (req, res, next) => {
    try {
        const data = await AdminService.setTenantStatus(req.params['id'], false);
        if (!data)
            throw NotFoundException({ message: 'Teacher not found' });
...
adminRouter.post('/tenants/:id/activate', async (req, res, next) => {
    try {
        const data = await AdminService.setTenantStatus(req.params['id'], true);
        if (!data)
            throw NotFoundException({ message: 'Teacher not found' });
...
adminRouter.post('/tenants/:id/subscription', async (req, res, next) => {
    try {
        const { planTier, durationMonths, studentsCount, paymentMethod, promoCode } = req.body;
        const data = await SubscriptionService.createSubscription(req.params['id'], { planTier, durationMonths, studentsCount, paymentMethod, promoCode });
...
adminRouter.get('/activity', async (req, res, next) => {
    try {
        const { page, limit, event, tenantId } = req.query;
...
adminRouter.get('/platform-settings', async (_req, res, next) => {
    try {
...
adminRouter.patch('/platform-settings/plan-prices', async (req, res, next) => {
    try {
...
adminRouter.patch('/promo-codes/:id/toggle', async (req, res, next) => {
    try {
        const data = await AdminService.togglePromoCode(req.params['id']);
...
adminRouter.delete('/promo-codes/:id', async (req, res, next) => {
    try {
        await AdminService.deletePromoCode(req.params['id']);
...
adminRouter.patch('/announcements/:id/toggle', async (req, res, next) => {
    try {
