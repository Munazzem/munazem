...
export class AIExamService {
    static async generateFromPDF(teacherId, pdfBuffer, examMeta, options) {
        ...
        if (!apiKey)
            throw BadRequestException({ message: 'مفتاح Groq API غير مُهيأ' });
        ...
        parsed.questions = parsed.questions.map((q) => ({
            ...q,
            type: typeAliasMap[q.type] ?? q.type?.toUpperCase() ?? q.type,
        }));
        // 4. Compute totalMarks
        const totalMarks = parsed.questions.reduce((sum, q) => sum + (q.marks ?? 2), 0);
...
//# sourceMappingURL=ai-exam.service.js.map