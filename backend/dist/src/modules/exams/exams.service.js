...
export class ExamsService {
    ...
    static async getExams(teacherId, query = {}) {
    ...
    // ── Get single exam ─────────────────────────────────────────────
    static async getExamById(examId, teacherId) {
        const exam = await ExamModel.findOne({ _id: examId, teacherId }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود' });
        return exam;
    }
    // ── Update exam (allowed only in DRAFT status) ───────────────────
    static async updateExam(examId, teacherId, data) {
        const exam = await ExamModel.findOne({ _id: examId, teacherId }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود' });
        ...
        return await ExamModel.findOneAndUpdate({ _id: examId, teacherId }, data, { new: true, runValidators: true }).lean();
    }
    // ── Publish exam ─────────────────────────────────────────────────
    static async publishExam(examId, teacherId) {
        const exam = await ExamModel.findOne({ _id: examId, teacherId }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود' });
        // Allow publishing even without questions
        return await ExamModel.findOneAndUpdate({ _id: examId, teacherId }, { status: ExamStatus.PUBLISHED }, { new: true }).lean();
    }
    // ── Delete exam ───────────────────────────────────────────────────
    static async deleteExam(examId, teacherId) {
        const exam = await ExamModel.findOneAndDelete({ _id: examId, teacherId, status: ExamStatus.DRAFT }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود أو لا يمكن حذف امتحان منشور' });
        return exam;
    }
    // ── Record single student result ─────────────────────────────────
    static async recordResult(teacherId, recordedBy, data) {
        const exam = await ExamModel.findOne({ _id: data.examId, teacherId }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود' });
        if (exam.status === ExamStatus.DRAFT)
            throw BadRequestException({ message: 'يجب نشر الامتحان أولاً' });
        ...
        const student = await StudentModel.findOne({ _id: data.studentId, teacherId }, { studentName: 1, groupId: 1 }).lean();
        if (!student)
            throw NotFoundException({ message: 'الطالب غير موجود أو لا ينتمي لك' });
        ...
        try {
            ...
            if (err.code === 11000)
                throw ConflictException({ message: 'تم تسجيل درجة هذا الطالب بالفعل' });
            throw err;
        }
    }
    // ── Batch record results for multiple students ───────────────────
    static async batchRecordResults(teacherId, recordedBy, data) {
        const exam = await ExamModel.findOne({ _id: data.examId, teacherId }).lean();
        if (!exam)
            throw NotFoundException({ message: 'الامتحان غير موجود' });
        if (exam.status === ExamStatus.DRAFT)
        ...
        const students = await StudentModel.find({ _id: { $in: studentIds }, teacherId }, { studentName: 1, groupId: 1, parentPhone: 1 } // parentPhone for WhatsApp
        ).lean();
    ...
    static async getExamResults(examId, teacherId) {
        ...
        const students = await StudentModel.find({ _id: { $in: studentIds } }, { parentPhone: 1 }).lean();
        const phoneMap = new Map(students.map(s => [s._id.toString(), s.parentPhone]));
    ...
    static async getStudentExamHistory(studentId, teacherId) {
