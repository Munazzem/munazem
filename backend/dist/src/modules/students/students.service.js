...
export class StudentService {
    ...
    static async createStudent(teacherId, data) {
        ...
        const letter = GRADE_LETTER[data.gradeLevel];
    ...
    static async bulkCreateStudents(teacherId, students) {
    ...
    static async getStudentsByTeacherId(teacherId, queryFilters) {
    ...
    static async updateStudent(studentId, teacherId, data) {
        ...
        try {
            const updatedStudent = await StudentModel.findOneAndUpdate({ _id: studentId, teacherId }, updatePayload, { new: true, runValidators: true }).lean();
            if (!updatedStudent)
                throw NotFoundException({ message: 'الطالب غير موجود' });
            ...
            if (error.code === 11000) {
                if (error.keyPattern?.barcode)
                    throw ConflictException({ message: 'الباركود مستخدم لطالب آخر' });
            }
            throw error;
        }
    }
