...
export class ParentService {
    static async lookupByPhone(parentPhone) {
        const phone = parentPhone.trim();
        if (!phone)
            throw BadRequestException({ message: 'رقم الهاتف مطلوب' });
        const students = await StudentModel.find({ parentPhone: phone }, { studentName: 1, gradeLevel: 1, groupId: 1, teacherId: 1,
            studentCode: 1, isActive: 1, parentName: 1 }).lean();
        if (!students.length) {
            throw NotFoundException({ message: 'لم يتم العثور على أي طالب مرتبط بهذا الرقم' });
        }
        const results = await Promise.all(students.map(student => ParentService.buildStudentSummary(student)));
        return results;
    }
    static async buildStudentSummary(student) {
        ...
        let presentCount = 0;
        let absentCount = 0;
        const attendanceHistory = snapshots.slice(0, 20).map((snap) => {
            const sid = studentId.toString();
            const isPresent = snap.presentStudents.some((s) => s.studentId.toString() === sid);
            const isAbsent = snap.absentStudents.some((s) => s.studentId.toString() === sid);
        ...
        for (const snap of snapshots.slice(20)) {
            const sid = studentId.toString();
            if (snap.presentStudents.some((s) => s.studentId.toString() === sid))
                presentCount++;
            else if (snap.absentStudents.some((s) => s.studentId.toString() === sid))
                absentCount++;
        }
        ...
        const payments = await TransactionModel.find({ teacherId, studentId, type: TransactionType.INCOME }, { category: 1, paidAmount: 1, discountAmount: 1, date: 1, description: 1 }).sort({ date: -1 }).lean();
        ...
        const hasActiveSubscription = subscriptions.some(s => new Date(s.date) >= monthStart && new Date(s.date) < monthEnd);
        // Exam results
        const examResults = await ExamResultModel.find({ studentId, teacherId }, { examId: 1, score: 1, totalMarks: 1, passingMarks: 1, date: 1, isPassed: 1 }).sort({ date: -1 }).lean();
        return {
