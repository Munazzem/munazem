...
export class PaymentsService {
    ...
    static async updateTransaction(teacherId, transactionId, data) {
        ...
        const updated = await withTransaction(async (session) => {
            const result = await TransactionModel.findByIdAndUpdate(transactionId, { $set: update }, { new: true, runValidators: true, session }).lean();
            ...
            if (data.amount !== undefined && data.amount !== transaction.paidAmount) {
                ...
                await Promise.all([
                    ...
                    DailyLedgerModel.findOneAndUpdate({ teacherId, date: day }, {
                        $inc: {
                    ...
                    MonthlyLedgerModel.findOneAndUpdate({ teacherId, year, month, 'dailySummaries.date': day }, {
                        $inc: {
                            totalIncome: isIncome ? delta : 0,
                            totalExpenses: isIncome ? 0 : delta,
                            netBalance: isIncome ? delta : -delta,
                    ...
                    DailyLedgerModel.findOneAndUpdate({ teacherId, date: day, 'transactions.transactionId': transaction._id }, { $set: { 'transactions.$.paidAmount': data.amount } }, { session }),
                ]);
            }
            return result;
        });
