...
paymentsRouter.put('/prices', authorizeRoles(UserRole.teacher), validate(upsertPriceSettingsSchema), async (req, res, next) => {
    try {
...
paymentsRouter.get('/prices', authorizeRoles(UserRole.teacher), async (req, res, next) => {
    try {
        const teacherId = req.user.userId;
...
paymentsRouter.post('/subscription', authorizeRoles(UserRole.assistant, UserRole.teacher), validate(recordSubscriptionSchema), async (req, res, next) => {
    try {
        const user = req.user;
...
paymentsRouter.post('/subscription/batch', authorizeRoles(UserRole.assistant, UserRole.teacher), validate(batchSubscriptionSchema), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
...
paymentsRouter.post('/notebook', authorizeRoles(UserRole.assistant, UserRole.teacher), validate(recordNotebookSaleSchema), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
...
paymentsRouter.post('/notebook/reserve', authorizeRoles(UserRole.assistant, UserRole.teacher), validate(reserveNotebookSchema), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
...
paymentsRouter.post('/notebook/deliver/:reservationId', authorizeRoles(UserRole.assistant, UserRole.teacher), validate(deliverNotebookSchema), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
        const result = await PaymentsService.deliverNotebook(teacherId, user.userId, req.params['reservationId'], req.body);
...
paymentsRouter.post('/expense', authorizeRoles(UserRole.teacher, UserRole.assistant), validate(recordExpenseSchema), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
...
paymentsRouter.get('/ledger/daily', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = resolveTeacherId(user);
...
paymentsRouter.get('/ledger/monthly', authorizeRoles(UserRole.teacher), async (req, res, next) => {
    try {
        const teacherId = req.user.userId;
        const now = new Date();
...
paymentsRouter.patch('/:id', (req, _res, next) => {
    try {
        const user = req.user;
...
}, validate(updateTransactionSchema), async (req, res, next) => {
    try {
        const teacherId = req.user.userId;
        const updated = await PaymentsService.updateTransaction(teacherId, req.params['id'], req.body);
