...
reportsRouter.get('/dashboard', authorizeRoles(UserRole.teacher, UserRole.assistant), // Allow Assistant
async (req, res, next) => {
    try {
        const user = req.user;
...
reportsRouter.get('/daily-summary', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = user.role === UserRole.assistant ? user.teacherId : user.userId;
...
reportsRouter.get('/daily-summary/pdf', authorizeRoles(UserRole.teacher, UserRole.assistant), async (req, res, next) => {
    try {
        const user = req.user;
        const teacherId = user.role === UserRole.assistant ? user.teacherId : user.userId;
        const date = req.query['date'];
...
reportsRouter.get('/student/:studentId', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const data = await ReportsService.getStudentReport(req.params['studentId'], teacherId);
...
reportsRouter.get('/student/:studentId/pdf', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const htmlString = await PdfService.generateStudentReportPdf(req.params['studentId'], teacherId);
        res.set({
            'Content-Type': 'text/html; charset=utf-8',
        });
        res.send(htmlString);
...
reportsRouter.get('/group/:groupId', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const data = await ReportsService.getGroupReport(req.params['groupId'], teacherId);
...
reportsRouter.get('/group/:groupId/pdf', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const htmlString = await PdfService.generateGroupReportPdf(req.params['groupId'], teacherId);
        res.set({
            'Content-Type': 'text/html; charset=utf-8',
        });
        res.send(htmlString);
...
reportsRouter.get('/group/:groupId/attendance-sheet', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const htmlString = await PdfService.generateGroupAttendanceSheetHtml(req.params['groupId'], teacherId);
        res.set({
            'Content-Type': 'text/html; charset=utf-8',
        });
        res.send(htmlString);
...
reportsRouter.get('/financial/monthly', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const now = new Date();
...
reportsRouter.get('/financial/monthly/pdf', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
        const now = new Date();
        const year = parseInt(req.query['year']) || now.getUTCFullYear();
...
reportsRouter.get('/unpaid-students', async (req, res, next) => {
    try {
        const teacherId = resolveTeacherId(req.user);
