...
export class PdfService {
    ...
    static wrapHtmlContent(title, content, centerName, logoUrl) {
     ...
     */
    static async generateStudentReportPdf(studentId, teacherId) {
        ...
        <body>
            ...
            <div class="info-section">
                <div class="info-block">
                    ...
                    <p><strong>كود الطالب:</strong> ${student.studentCode ?? '—'}</p>
                </div>
                <div class="info-block">
    ...
    // ─────────────────────────────────────────────────────────────────
    static async generateMonthlyFinancialPdf(teacherId, year, month) {
        const teacher = await UserModel.findById(teacherId).lean();
    ...
    // 3. Daily Summary PDF
    // ─────────────────────────────────────────────────────────────────
    static async generateDailySummaryPdf(teacherId, dateStr) {
        const teacher = await UserModel.findById(teacherId).lean();
    ...
    // ─────────────────────────────────────────────────────────────────
    static async generateGroupReportPdf(groupId, teacherId) {
        const teacher = await UserModel.findById(teacherId).lean();
        ...
        const content = `
            ...
            ? `<p><strong>المواعيد:</strong> ${report.group.schedule.map((s) => `${s.day ?? ''} ${s.time ?? ''}`).join(' | ')}</p>`
            ...
            report.revenue.breakdown.map((r) => `
                        <tr>
                            <td>${r._id}</td>
                            <td>${r.count}</td>
    ...
    // ─────────────────────────────────────────────────────────────────
    static async generateSessionAttendancePdf(sessionId, teacherId) {
        const teacher = await UserModel.findById(teacherId).lean();
    ...
    // ─────────────────────────────────────────────────────────────────
    static async generateGroupAttendanceSheetHtml(groupId, teacherId) {
        ...
        const attendanceMap = {}; // sessionId -> { studentId: true }
        ...
        const activeSubSet = new Set(subscriptions.map((id) => id.toString()));
