...
export class UserService {
    static async createUser(creatorRole, creatorId, data) {
    ...
    static async getUsers(requesterRole, requesterId, query) {
        let filter = {};
    ...
    static async updateUser(requesterRole, requesterId, targetUserId, data) {
    ...
    static async paySalary(teacherId, assistantId, amount, notes) {
        ...
        if (!assistant)
            throw NotFoundException({ message: 'المساعد غير موجود أو لا ينتمي إليك' });
        // Reuse PaymentsService.recordExpense to keep ledger logic consistent
        const tx = await PaymentsService.recordExpense(teacherId, teacherId, // createdBy = the teacher
        {
    ...
    static async deleteUser(requesterRole, requesterId, targetUserId) {
        const targetUser = await UserModel.findById(targetUserId);
        if (!targetUser) {
            throw BadRequestException({ message: 'المستخدم غير موجود' });
        }
