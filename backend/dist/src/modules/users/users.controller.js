...
class UserController {
    static async createUser(req, res, next) {
        try {
            ...
            const newUser = await UserService.createUser(currentUser.role, currentUser.userId, req.body);
    ...
    static async getUsers(req, res, next) {
        try {
            const currentUser = req.user;
            const users = await UserService.getUsers(currentUser.role, currentUser.userId, req.query);
    ...
    static async updateUser(req, res, next) {
        try {
            ...
            const updatedUser = await UserService.updateUser(currentUser.role, currentUser.userId, targetUserId, req.body);
    ...
    static async deleteUser(req, res, next) {
        try {
            const currentUser = req.user;
            const targetUserId = req.params.id;
            const result = await UserService.deleteUser(currentUser.role, currentUser.userId, targetUserId);
...
router.post('/:id/pay-salary', authorizeRoles(UserRole.teacher), validate(paySalarySchema), async (req, res, next) => {
    try {
