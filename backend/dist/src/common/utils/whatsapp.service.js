...
function updateTeacherWA(teacherId, update) {
    UserModel.updateOne({ _id: teacherId }, update).catch((err) => {
 ...
 */
export async function initializeClientForTeacher(teacherId) {
    ...
    client.on('message', async (msg) => {
        ...
        if (body === 'إلغاء' || body === 'الغاء' || body === 'stop') {
            ...
            if (!phone)
                return;
            try {
                await OptOutModel.updateOne({ phone: phone, teacherId: teacherId }, { $set: { phone: phone, teacherId: teacherId } }, { upsert: true });
                ...
                logger.error('whatsapp_opt_out_failed', {
                    teacherId, phone, error: err.message,
                });
            }
        }
    });
 ...
 */
export async function sendWhatsAppMessage(rawParentPhone, message, teacherId) {
    const entry = clientsPool.get(teacherId);
    if (!entry || !entry.ready) {
        throw new Error(`WhatsApp client for teacher ${teacherId} is not ready — job will be retried by BullMQ`);
    }
...
// ─── Pool status helper ───────────────────────────────────────────────────────
export function getClientStatus(teacherId) {
    const entry = clientsPool.get(teacherId);
    if (!entry)
        return 'disconnected';
 ...
 */
export async function destroyClientForTeacher(teacherId) {
    const entry = clientsPool.get(teacherId);
    ...
    try {
        ...
        const sessionDir = path.join(process.cwd(), '.wwebjs_auth', `session-session-${teacherId}`);
        await rm(sessionDir, { recursive: true, force: true });
        logger.info('whatsapp_session_deleted', { teacherId, sessionDir });
    }
    catch (err) {
        // Non-fatal: session folder may not exist yet
        logger.warn('whatsapp_session_delete_failed', { teacherId, error: err.message });
    }
 ...
 */
export async function autoReconnectClients() {
    try {
        const teachers = await UserModel.find({ whatsappStatus: 'connected', role: UserRole.teacher }, { _id: 1 }).lean();
        ...
        logger.error('whatsapp_auto_reconnect_failed', { error: err.message });
    }
}
