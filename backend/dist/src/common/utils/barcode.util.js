...
export class BarcodeUtil {
     ...
     */
    static async generateBase64Barcode(text) {
        if (!text || !text.trim()) {
            throw new Error("Barcode text cannot be empty or just whitespace");
        }
        return new Promise((resolve, reject) => {
            bwipjs.toBuffer({
                ...
                if (err) {
                    console.error(`Barcode Generation Error for text [${text}]:`, err);
                    return reject(new Error("Failed to generate barcode image"));
                }
                else {
