...
export const updateAttendanceSchema = z.object({
  body: z.object({
    status:       z.nativeEnum(AttendanceStatus, { error: () => ({ message: 'حالة الحضور غير صحيحة' }) }).optional(),
    homeworkDone: z.boolean().optional(),
    notes:        z.string().max(300).optional(),
  }).refine((data) => data.status !== undefined || data.homeworkDone !== undefined || data.notes !== undefined, {
    message: 'يجب إرسال حالة الحضور أو حالة الواجب أو الملاحظات',
  }),
});
