 ...
 */
export async function generatePaymentReminders(teacherId?: string): Promise<void> {
    ...
    for (const teacher of teachers) {
        try {
            ...
            const optedOut = new Set(
                (await OptOutModel.find({ teacherId: teacher._id }, { phone: 1 }).lean())
                    .map(o => o.phone),
            );
            ...
            for (const group of groups) {
                ...
                if (secondSession.getTime() !== tomorrow.getTime()) continue;
                ...
                const unpaidStudents = await StudentModel.find(
                    ...
                    { studentName: 1, parentPhone: 1, gradeLevel: 1 },
                ).lean();
                ...
                for (const student of unpaidStudents) {
                    ...
                    if (normalizedPhone.startsWith('01')) normalizedPhone = '2' + normalizedPhone;
                    else if (!normalizedPhone.startsWith('20') && normalizedPhone.length === 10)
                        normalizedPhone = '20' + normalizedPhone;

                    // Check opt-out
                    if (optedOut.has(normalizedPhone)) continue;
