...
export class GroupCardsPdfService {
    static async generateGroupCardsHtml(groupId: string, teacherId: string): Promise<string> {
        const group = await GroupModel.findOne({ _id: groupId, teacherId }).lean();
        if (!group) {
            throw BadRequestException({ message: 'المجموعة غير موجودة' });
        }
        ...
        const students = await StudentModel.find({ groupId, teacherId }).lean();
        if (students.length === 0) {
            throw BadRequestException({ message: 'لا يوجد طلاب في هذه المجموعة' });
        }

        const studentsWithQr = await Promise.all(
            students.map(async (stu) => {
                const qrValue = stu.barcode || stu.studentCode || stu._id.toString();
        ...
        const centerLabel = teacher.centerName || 'اسم السنتر';
        const centerLogo = teacher.logoUrl ? `<img src="${teacher.logoUrl}" alt="Center Logo" />` : '';
                    ...
                    <div class="header">
                        <div class="header-top">
                            <div class="brand-item platform-brand">
                                ...
                                <img src="/logo.png" alt="منظم" onerror="this.style.display='none'" />
                            </div>
        ...
        return `
            ...
            <body>
                <div class="no-print" style="text-align: center; margin: 20px;">
                    <button onclick="window.print()" style="padding: 10px 20px; font-size: 16px; font-family: 'Cairo'; cursor: pointer; background-color: #2563eb; color: white; border: none; border-radius: 5px;">
                        طباعة الكروت
                    </button>
                </div>
